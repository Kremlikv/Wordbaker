 ...
 *
 * - Voices: fixed Standard voices (Czech=female; Foreign=male) as provided by you
...
// Register presets (prosody). Keys are used in query/POST; labels are user-facing.
// Tuned to be audibly distinct while still natural with Standard voices.
$STYLE_PRESETS = [
  'bass' => [
    'label'  => 'Bass',
    'rate'   => '96%',
    'pitch'  => '-6st',   // significantly lower
    'volume' => '+2dB'
  ],
  'baritone' => [
    'label'  => 'Baritone',
    'rate'   => '100%',
    'pitch'  => '-3st',   // moderately lower
    'volume' => '+1dB'
  ],
  'tenor' => [
    'label'  => 'Tenor',
    'rate'   => '102%',
    'pitch'  => '+3st',   // moderately higher
    'volume' => '+0dB'
  ],
  'countertenor' => [
    'label'  => 'Counter‑tenor',
    'rate'   => '98%',
    'pitch'  => '+6st',   // significantly higher
    'volume' => '-1dB'
  ],
];

// Short preview phrases
$PREVIEW_TEXT = [
...
if (isset($_GET['preview']) && $_GET['preview']==='1') {
  ...
  $lang  = trim($_GET['lang'] ?? '');
  $styleKey = trim($_GET['style'] ?? 'baritone'); // default to baritone for “male” feel
...
function render_style_picker(string $table, string $srcKey, string $srcCode, string $tgtKey, string $tgtCode, string $v1, string $v2, array $STYLE_PRESETS) {
  ...
  foreach ($STYLE_PRESETS as $key=>$st) {
    echo "<option value='".htmlspecialchars($key,ENT_QUOTES)."'".($key==='tenor'?' selected':'').">".htmlspecialchars($st['label'])."</option>";
  }
  echo "</select>";
  ...
  foreach ($STYLE_PRESETS as $key=>$st) {
    echo "<option value='".htmlspecialchars($key,ENT_QUOTES)."'".($key==='baritone'?' selected':'').">".htmlspecialchars($st['label'])."</option>";
  }
  echo "</select>";
...
$styleKey1      = trim($_POST['style1'] ?? 'tenor');     // slightly higher fits female sensibly
$styleKey2      = trim($_POST['style2'] ?? 'baritone');  // male default
...
//////////////////// Prepare GAP once (use L1 style for coherence) ////////////////////

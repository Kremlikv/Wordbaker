 ...
 *
 * Usage:
 *   // Default (manage mode, posts to main.php)
 *   include 'file_explorer.php';
 *
 *   // QUIZ mode (same menus, posts to current page)
 *   $EXPLORER_MODE = 'quiz';
 *   include 'file_explorer.php';
 *
 * Requires:
 *   - $folders           // array: left pane folders => tables
 *   - $folderData        // array: folder => [{ table:"full_table_name", display:"folder/_/sub/_/file" }, ...]
 *   - $selectedFullTable, $column1, $column2
 */
$__MODE = isset($EXPLORER_MODE) ? strtolower(trim($EXPLORER_MODE)) : 'manage';
$__POST_TARGET = ($__MODE === 'quiz') ? '' : 'main.php'; // '' => current page
?>
...
/* Context menus */
.folder-context-menu,.subfolder-context-menu{
  position:absolute; display:none; z-index:9999; background:#fff; border:1px solid #e2e8f0;
  border-radius:8px; box-shadow:0 10px 20px rgba(0,0,0,.08); min-width:230px; padding:6px;
}
.folder-context-menu button,.subfolder-context-menu button{
  width:100%; border:0; background:transparent; text-align:left; padding:8px 10px;
  cursor:pointer; border-radius:6px; font-size:14px;
}
.folder-context-menu button:hover,.subfolder-context-menu button:hover{ background:#f1f5f9; }
...
(function(){
  ...
  document.getElementById('shareFolderBtn').addEventListener('click', function(){
    ...
    if (!confirm('Share all tables in "'+targetFolder+'"? (visible under Shared)')) return;
...
(function () {
  ...
  document.addEventListener('contextmenu', function (e) {
    ...
    document.getElementById('shareSubToUserBtn').disabled= isSharedRoot;
  ...
  document.getElementById('renameSubBtn').addEventListener('click', function(){
    ...
    subForm.root_folder.value  = root;
    subForm.subpath.value      = sub;
    subForm.new_name.value     = newName.trim();
    subForm.submit();
  });
